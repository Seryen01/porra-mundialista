...
function AllBonusSection({ currentUserId }: { currentUserId?: string }) {
  ...
  return (
    <div className="ab-section card animate-in stagger-2">
      <div className="ab-header" onClick={() => setCollapsed(c => !c)} style={{ cursor: "pointer" }}>
        <h2 className="ab-title">👥 Picks de todos</h2>
        <div className="ab-header-right">
          <span className="ab-sub">{bonuses.length} participantes</span>
          {collapsed ? <ChevronDown size={16} className="ab-chevron" /> : <ChevronUp size={16} className="ab-chevron" />}
        </div>
      </div>
      ...
      {!collapsed && (
        <div className="ab-table-wrap">
          <table className="ab-table">
            <thead>
              <tr>
                <th className="ab-th ab-th-user">Jugador</th>
                {COLS.map(c => (
